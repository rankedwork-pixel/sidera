...
async def _extract_org_structure(
    ...
    doc_content = _prepare_doc_content(docs)
    ...
    try:
        ...
        logger.warning("bootstrap.extract_org_error", error=str(exc))
        return [], [], 0.0
...
async def _extract_skills(
    ...
    try:
        ...
        logger.warning("bootstrap.extract_skills_error", error=str(exc))
        return [], 0.0
...
async def _extract_context(
    ...
    try:
        ...
        logger.warning("bootstrap.extract_context_error", error=str(exc))
        return {"memories": []}, 0.0
    ...
    return {
        "role_goals": parsed.get("role_goals", []),
        "role_principles": parsed.get("role_principles", []),
        "department_vocabulary": parsed.get("department_vocabulary", []),
        "memories": memories,
    }, cost
